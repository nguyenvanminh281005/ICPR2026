...
Each track folder contains:
    - lr-*.png/jpg: Low-resolution images (5 frames)
    - hr-*.png/jpg: High-resolution images (5 frames, train only)
    - annotations.json: Label file with 'plate_text' field
"""
import glob
import json
import os
...
DEFAULT_TRAIN_DIR = os.path.join(DEFAULT_DATA_ROOT, "train")
DEFAULT_TEST_DIR = os.path.join(DEFAULT_DATA_ROOT, "test")
DEFAULT_VAL_SPLIT_FILE = os.path.join(DEFAULT_DATA_ROOT, "val_tracks.json")
    ...
    def __init__(
        ...
        Args:
            root_dir: Root directory containing track folders.
                      Defaults to 'data/train' for train/val mode, 'data/test' for test mode.
            mode: 'train' or 'val'.
            split_ratio: Train/val split ratio (default 0.9 = 90% train, 10% val).
            img_height: Target image height for resizing.
            img_width: Target image width for resizing.
            char2idx: Character to index mapping for CTC encoding.
            val_split_file: Path to validation split JSON file (default: 'data/val_tracks.json').
        ...
        Example usage:
            # Training dataset
            train_ds = MultiFrameDataset(root_dir='data/train', mode='train', char2idx=char2idx)
            
            # Validation dataset
            val_ds = MultiFrameDataset(root_dir='data/train', mode='val', char2idx=char2idx)
            
            # Test dataset (no labels)
            test_ds = MultiFrameDataset(root_dir='data/test', is_test=True)
        """
        # Set default paths based on mode
        if root_dir is None:
            root_dir = DEFAULT_TEST_DIR if is_test else DEFAULT_TRAIN_DIR
        if val_split_file is None:
